...
from scanindex.core.kie.json_utils import upgrade_ocr_data_in_place
from scanindex.core.kie.common import read_json, utc_now_iso
...
def labeling_instructions() -> list[str]:
    return [
        "Keep `text` exactly as OCR surface text. Do not invent missing prefixes or rewrite OCR surface text.",
                "Truoc khi audit/label mot batch, bat buoc doc tai lieu trong `train-convert/kie/train_kie/` va runtime core `kie_core/` theo dung thu tu do.",
...
Quy tac bat buoc:
- Truoc khi lam, phai doc tai lieu trong `train-convert/kie/train_kie/` va runtime core `kie_core/`.
