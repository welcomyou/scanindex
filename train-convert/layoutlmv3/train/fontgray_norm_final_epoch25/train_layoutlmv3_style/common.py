...
PROJECT_SCHEMA_VERSION = "layoutlmv3_fontgray_linebucket_norm_kie_project_v3"
STYLE_BASE_TYPE_VOCAB_SIZE = 64
LINE_POSITION_BUCKET_COUNT = 16
LINE_EXACT_PREFIX_BUCKETS = 12
STYLE_TYPE_VOCAB_SIZE = STYLE_BASE_TYPE_VOCAB_SIZE * LINE_POSITION_BUCKET_COUNT

_LINE_ID_RE = re.compile(r"^p(?P<page>\d+)_l(?P<line>\d+)$", re.IGNORECASE)
...
def style_type_ids(font_size: list[float], fg_gray: list[float], word_height: list[float]) -> list[int]:
    ...
    for fs, gray, height in zip(font_size, fg_gray, word_height):
        ...
        ids.append(min(STYLE_BASE_TYPE_VOCAB_SIZE - 1, rel * 16 + g * 4 + h))
    return ids
...
def _parse_line_index(line_id: Any) -> int | None:
    if line_id is None:
        return None
    match = _LINE_ID_RE.match(str(line_id).strip())
    if not match:
        return None
...
def _line_bucket_from_rank(rank: int, line_count: int) -> int:
    if rank < 0:
        return 0
    if rank < LINE_EXACT_PREFIX_BUCKETS:
        return 1 + rank
    tail_buckets = max(1, LINE_POSITION_BUCKET_COUNT - 1 - LINE_EXACT_PREFIX_BUCKETS)
    tail_count = max(1, line_count - LINE_EXACT_PREFIX_BUCKETS)
    tail_rank = max(0, rank - LINE_EXACT_PREFIX_BUCKETS)
    return 1 + LINE_EXACT_PREFIX_BUCKETS + min(tail_buckets - 1, int(tail_rank * tail_buckets / tail_count))


def line_position_bucket_ids(line_ids: list[Any]) -> list[int]:
...
def combine_style_line_type_ids(style_ids: list[int], line_bucket_ids: list[int]) -> list[int]:
    out: list[int] = []
    for style_id, line_bucket_id in zip(style_ids, line_bucket_ids):
        style = min(STYLE_BASE_TYPE_VOCAB_SIZE - 1, max(0, int(style_id)))
        line_bucket = min(LINE_POSITION_BUCKET_COUNT - 1, max(0, int(line_bucket_id)))
        out.append(line_bucket * STYLE_BASE_TYPE_VOCAB_SIZE + style)
    return out
...
def add_layoutlmv3_style_features(row: dict[str, Any]) -> dict[str, Any]:
    ...
    line_ids = list(row.get("line_ids", []))
    debug = relative_style_debug(font_size, fg_gray, word_height)
    style_ids = style_type_ids(font_size, fg_gray, word_height)
    line_bucket_ids = line_position_bucket_ids(line_ids)
    if len(line_bucket_ids) != len(style_ids):
        line_bucket_ids = (line_bucket_ids + [0] * len(style_ids))[: len(style_ids)]
    out = dict(row)
    out["layoutlmv3_style_base_id"] = style_ids
    out["layoutlmv3_line_bucket_id"] = line_bucket_ids
    out["layoutlmv3_style_type_id"] = combine_style_line_type_ids(style_ids, line_bucket_ids)
...
def build_rows_for_doc_with_style(
    ...
    for row in out:
        ...
        stats["line_feature_words"] += len(row.get("layoutlmv3_line_bucket_id", []))
        stats["line_unknown_bucket_words"] += sum(1 for value in row.get("layoutlmv3_line_bucket_id", []) if int(value) == 0)
...
def summarize_dataset_rows(rows_by_split: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    for split, rows in rows_by_split.items():
        ...
        line_unknown = sum(sum(1 for value in row.get("layoutlmv3_line_bucket_id", []) if int(value) == 0) for row in rows)
        style[split] = {
            "words": words,
            "style_nonzero_rate": nonzero / words if words else 0.0,
            "line_unknown_bucket_rate": line_unknown / words if words else 0.0,
...
def write_style_report(project_root: str | Path) -> None:
    ...
    lines = [
        "# LayoutLMv3 Font/Gray/Line Normalized Style KIE Report",
        "",
        f"Generated: {now_iso()}",
        "",
        "## Feature Contract",
        "- Source: canonical OCR JSON `words` and `lines` metadata.",
        "- Fields: `font_size`, `fg_gray`, `word_height`, `line_ids`, `confidence`, `content_type` mapped by exact `word_id`.",
        "- Model input: combined style + stable line-position bucket in `token_type_ids`; no page image/pixel input and no sequence-length inflation.",
        "- Normalization: `font_size`, `fg_gray`, and `word_height` are bucketed relative to each page median.",
        f"- Line feature: parsed line ids such as `p0_l3`; first {LINE_EXACT_PREFIX_BUCKETS} lines keep exact rank, later lines are bucketed.",
    ...
    if manifest:
        ...
        for split, stats in (manifest.get("summary", {}).get("style") or {}).items():
            lines.append(
                ...
                f"line_unknown={stats.get('line_unknown_bucket_rate', 0):.3f}, "
