...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--doc-meta-json", help="Optional LightGBM-style doc meta JSON. Uses source_canonical_json/doc_id/selected_pages unless overridden.")
...
def main() -> None:
    ...
    doc_id = None
    if args.doc_meta_json:
        with Path(args.doc_meta_json).open("r", encoding="utf-8") as f:
            doc_meta = json.load(f)
        canonical_json = Path(doc_meta.get("source_canonical_json") or args.input)
        selected_pages = selected_pages or doc_meta.get("selected_pages")
        doc_id = str(doc_meta.get("doc_id") or "") or None
    else:
        canonical_json = resolve_canonical(args.input)
    rows = rows_from_canonical_with_style(canonical_json, selected_pages=selected_pages, doc_id=doc_id)
    if not rows:
    ...
    payload = {
        ...
        "doc_meta_json": str(Path(args.doc_meta_json).resolve()) if args.doc_meta_json else None,
