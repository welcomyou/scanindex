...
from train_layoutlmv3_style.hf_utils import decode_schema_spans, predict_onnx, predict_pytorch


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate LayoutLMv3 font/gray style KIE on a labeled batch prefix.")
...
def main() -> None:
    ...
    variants = [("pytorch", None)] if args.backend == "pytorch" else [parse_variant(raw) for raw in args.onnx or []]
    for variant_name, variant_path in variants:
        report["variants"][variant_name] = {
            "path": str(variant_path.resolve()) if variant_path else str(Path(args.model_path).resolve())
        }
        ...
        for split, rows in rows_by_split.items():
            try:
                ...
                report["splits"][variant_name][split]["latency"] = latency if split == "all" else split_latency
                write_jsonl(
