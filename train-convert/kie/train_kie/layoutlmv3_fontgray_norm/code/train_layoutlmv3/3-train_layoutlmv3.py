...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune LayoutLMv3-base for word-level BIO KIE.")
    parser.add_argument("--project-root", required=True)
    parser.add_argument("--model-name", default="microsoft/layoutlmv3-base")
    parser.add_argument("--output-dir", help="Default: <project-root>/models/layoutlmv3_base_run1")
    ...
    parser.add_argument("--hard-example-jsonl", action="append", help="Error JSONL files used to oversample bad docs.")
    parser.add_argument("--hard-example-repeat", type=int, default=1)
    parser.add_argument("--focus-fields", nargs="*", default=[], help="Fields whose rows should be oversampled, e.g. RECIPIENTS DOC_SUBJECT.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Use 20 train/val docs, one epoch, and a few steps.")
    parser.add_argument("--max-steps", type=int, default=-1)
    parser.add_argument("--seed", type=int, default=42)
    return parser.parse_args()


class LayoutLMv3KIEDataset(Dataset):
    def __init__(
        self,
            ...
            for chunk_index in range(chunk_count):
                ...
                for word_id in word_ids:
                    if word_id is None:
                        ...
                        continue
                    if subword_label_strategy == "first" and word_id in seen_words:
                        aligned_labels.append(-100)
                    else:
...
def prediction_metrics_fn(eval_dataset: LayoutLMv3KIEDataset, id2label: dict[int, str]):
    def compute(eval_pred) -> dict[str, float]:
        ...
        for chunk_index, meta in enumerate(eval_dataset.metadata):
        ...
        for row_index, row in enumerate(eval_dataset.rows):
            labels: list[str] = []
            scores: list[float] = []
...
def class_weights_from_rows(rows: list[dict[str, Any]], label2id: dict[str, int]) -> torch.Tensor:
...
class WeightedLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.loss_name == "focal":
            ...
            weighted = ((1 - pt) ** self.focal_gamma * flat_loss * token_weights)[valid]
...
def row_has_focus_field(row: dict[str, Any], focus_fields: set[str]) -> bool:
    ...
    for label in row.get("labels", []):
        if "-" not in label:
            continue
        if label.split("-", 1)[1] in focus_fields:
            return True
    return False
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else dirs["models"] / "layoutlmv3_base_run1"
    ...
    if not train_rows or not val_rows:
        raise SystemExit("Dataset is missing train/val rows. Run 1-build_dataset.py first.")
    ...
    train_rows = oversample_rows(
        train_rows,
        hard_doc_ids=hard_doc_ids,
        hard_repeat=args.hard_example_repeat,
    ...
    train_dataset = LayoutLMv3KIEDataset(
        train_rows,
        tokenizer,
        label2id,
    ...
    val_dataset = LayoutLMv3KIEDataset(
        val_rows,
        tokenizer,
        label2id,
        max_length=args.max_length,
    ...
    class_weights = class_weights_from_rows(train_rows, label2id) if args.loss in {"weighted_ce", "focal"} else None
    ...
    write_json(
        output_dir / "layoutlmv3_kie_config.json",
        {
